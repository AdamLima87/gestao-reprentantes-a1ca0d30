...
BEGIN
  ...
  ELSIF (v_rep.id IS NOT NULL AND v_rep.tipo = 'interno') OR COALESCE(v_pedido.jefferson_participou, false) THEN
    ...
    -- Se já existir uma NF posterior do mesmo cliente, esta é necessariamente "novo" (cronologicamente primeira)
    IF v_ultima IS NOT NULL AND v_ultima >= v_ref_date THEN
      v_tipo_int := 'interno_novo'; v_pct_int := 1.5;
    END IF;
  ...
  -- Só atualiza ultima_compra_at se a data desta NF for mais recente que o registro atual
...
BEGIN
  ...
  -- Processar NF-es em ordem cronológica real (data_nfe) para classificação correta
