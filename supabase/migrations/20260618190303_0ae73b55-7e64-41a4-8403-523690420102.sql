...
BEGIN
  ...
  IF v_pedido.representante_id IS NOT NULL THEN
    ...
    v_pct_ext := COALESCE(v_pct_ext, v_rep.percentual_padrao, 5.0);
  ...
  ELSE
    IF v_jefferson_id IS NOT NULL THEN
      ...
      INSERT INTO public.comissoes (
        ...
        NEW.id, NEW.pedido_id, v_jefferson_id,
        v_tipo_int, COALESCE(v_pct_int, 1.5), NEW.valor_nfe,
        ROUND(NEW.valor_nfe * COALESCE(v_pct_int, 1.5) / 100, 2),
        NEW.mes_ref, NEW.ano_ref
      );
    END IF;
  END IF;
